...
class ResponseBuffer:
    def __init__(self, timeout: float = 8.0, min_responses: int = 1):
        ...
        self.chunk_buffer: Dict[str, str] = {}
    ...
    async def add_chunk(self, participant_id: str, chunk: str):
        """Add a streaming chunk for a participant."""
        async with self.lock:
            if participant_id not in self.chunk_buffer:
                self.chunk_buffer[participant_id] = ""
            self.chunk_buffer[participant_id] += chunk

    async def get_response(self, participant_id: str) -> str:
        """Get and clear the buffered response for a participant."""
        async with self.lock:
            response = self.chunk_buffer.pop(participant_id, "")
        return response
    ...
    async def add_response(self, request_id: str, model_id: str, response: str):
        async with self.lock:
            ...
            self.buffer[request_id]["responses"][model_id] = {
                "response": response,
                "timestamp": time.time(),
            }
    ...
    async def check_completion(self, request_id: str):
        async with self.lock:
            if request_id not in self.buffer:
                return
            if len(self.buffer[request_id]["responses"]) >= self.min_responses:
    ...
    async def publish_results(self, request_id: str):
        ...
        if data["responses"]:
            ...
            self.monitoring["avg_response_time"] = (
                total * self.monitoring["avg_response_time"] + elapsed
            ) / (total + 1)
            self.monitoring["completed"] += 1
        callback = self.callbacks.pop(request_id, None)
        if callback:
            asyncio.create_task(callback(data["responses"]))

    def get_stats(self) -> Dict[str, Any]:
        return {
            **self.monitoring,
            "success_rate": self.monitoring["completed"]
            / max(self.monitoring["total_requests"], 1),
            "active_requests": len(self.buffer),
        }
